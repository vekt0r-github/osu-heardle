 ...
 * - warns if visiting port 5727 while running hot reloader
 */

class NodeSetupError extends Error {}
let routeChecked = false;

// poke port 8727 to see if 'npm run hotloader' was possibly called
...
module.exports = {
  ...
  checkRoutes: (req, res, next) => {
    if (!routeChecked && req.url === "/") {
      // if the server receives a request on /, we must be on port 5727 not 8727
      if (!fs.existsSync("./client/dist/bundle.js")) {
        throw new NodeSetupError(
          "Couldn't find bundle.js! If you want to run the hot reloader, make sure 'npm run hotloader'\n" +
            "is running and then go to http://localhost:8727 instead of port 5727.\n" +
        ...
        if (active) {
          console.log(
            "Warning: It looks like 'npm run hotloader' may be running. Are you sure you don't want\n" +
              "to use the hot reloader? To use it, visit http://localhost:8727 and not port 5727"
          );
        }
      });
